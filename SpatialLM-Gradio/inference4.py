...
# Try to import the actual modules, fallback to implementations if needed
try:
...
Provide a clear, concise description in simple English suitable for navigation assistance: [/INST]"""
    ...
    try:
        ...
        return f"Navigation Description: The space contains multiple walls, doors, and windows. {technical_layout_text[:200]}..."
...
def visualize_layout_with_rerun(point_cloud_file, layout_text, visualization_radius=0.01, max_visualization_points=1000000):
    """
    Proper implementation of point cloud visualization using Rerun
    """
    try:
        logger.info(f"Starting visualization for {point_cloud_file}")
        
        # Create a unique recording name based on timestamp
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        recording_name = f"spatial_scene_{timestamp}"
        
        # Initialize Rerun recording
        rr.init(recording_name, spawn=False)
        
        # Create the recording file path
        recording_path = os.path.join(visualization_dir, f"{recording_name}.rrd")
        rr.save(recording_path)
        
        # Load and process point cloud
        if MODULES_AVAILABLE and os.path.exists(point_cloud_file):
            logger.info("Loading point cloud for visualization...")
            point_cloud = load_o3d_pcd(point_cloud_file)
            points, colors = get_points_and_colors(point_cloud)
            
            # Subsample points if too many
            if len(points) > max_visualization_points:
                indices = np.random.choice(len(points), max_visualization_points, replace=False)
                points = points[indices]
                colors = colors[indices]
                logger.info(f"Subsampled to {max_visualization_points} points")
            ...
            rr.log(
                ...
                rr.Points3D(
                    positions=points,
                    colors=colors,
                    radii=visualization_radius
                )
            )
            
            # Add coordinate system
            rr.log("world/coordinate_system", rr.Arrows3D(
                origins=[[0, 0, 0], [0, 0, 0], [0, 0, 0]],
                vectors=[[1, 0, 0], [0, 1, 0], [0, 0, 1]],
                colors=[[255, 0, 0], [0, 255, 0], [0, 0, 255]]
            ))
            
            logger.info(f"Visualized {len(points)} points")
            
        else:
            # Fallback: create dummy visualization
            logger.warning("Creating dummy visualization due to missing modules or file")
            rr.set_time_sequence("frame", 0)
            rr.log(
                "world/points",
                rr.Points3D(
                    positions=np.random.rand(100, 3) * 5,
                    colors=np.random.rand(100, 3),
                    radii=0.05
                )
            )
        
        # Add layout information as text
        if layout_text:
            rr.log("world/layout_info", rr.TextDocument(layout_text[:500] + "..." if len(layout_text) > 500 else layout_text))
            
        logger.info(f"Visualization saved to {recording_path}")
        return recording_path
        
    except Exception as e:
        logger.error(f"Visualization failed: {e}")
        # Create a minimal fallback recording
        try:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            recording_name = f"fallback_scene_{timestamp}"
            rr.init(recording_name, spawn=False)
            recording_path = os.path.join(visualization_dir, f"{recording_name}.rrd")
            rr.save(recording_path)
            ...
            rr.log(
                "world/error",
                rr.TextDocument(f"Visualization Error: {str(e)}")
            )
    ...
    with gr.Blocks(title="SpatialLM Gradio | 3D Scene Understanding with Large Language Model ") as demo:
        ...
        def process_latest_pcd(last_processed_pcd_file, model_path, custom_prompt, top_k, top_p, temperature, num_beams, max_new_tokens, visualization_radius, max_visualization_points):
            ...
            if not os.path.exists(pcd_dir):
                return last_processed_pcd_file, None, None, None, None, None, None, None
            ...
            if latest_pcd_file != last_processed_pcd_file:
                ...
                try:
                    ...
                    return (latest_pcd_file, frame, latest_pcd_file, visualization_radius, 
                           max_visualization_points, technical_layout_str, simplified_layout_str, audio_path)
        ...
        def safe_visualize_layout(pcd_path, layout_text, radius, max_points):
            try:
                ...
                logger.error(f"Visualization failed: {e}")
                gr.Warning(f"3D visualization failed: {e}")
                return None
