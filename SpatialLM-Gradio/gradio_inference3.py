...
try:
    ...
    logging.warning(f"Could not import spatiallm utilities: {e}")
    # Define fallbacks
    def load_o3d_pcd(path):
        return o3d.io.read_point_cloud(path)
    def get_points_and_colors(pcd):
    ...
    def visualize_layout_with_rerun(*args): 
        # Create a dummy visualization file for the mock
        dummy_viz_dir = os.path.join(output_dir, "visualizations")
        os.makedirs(dummy_viz_dir, exist_ok=True)
        dummy_file = os.path.join(dummy_viz_dir, "dummy_visualization.txt")
        with open(dummy_file, "w") as f:
            f.write("Mock visualization data")
        return dummy_file
...
def simplify_layout_description(technical_layout_text):
    """
    Convert technical layout coordinates to human-readable navigation instructions.
    Truncate if too long for the model context.
    """
    simplify_model = get_simplification_model()
    if simplify_model is None:
        logger.warning("Simplification model not loaded, returning technical text")
        return technical_layout_text
    
    # Truncate the text if it's too long (keep the most important parts)
    max_tokens = 3500  # Leave some room for the prompt
    if len(technical_layout_text.split()) > max_tokens:
        logger.warning(f"Truncating layout text from {len(technical_layout_text.split())} to {max_tokens} tokens")
        # Keep the beginning and end (where important info often is)
        words = technical_layout_text.split()
        truncated_text = ' '.join(words[:max_tokens//2] + ["..."] + words[-max_tokens//2:])
    else:
        truncated_text = technical_layout_text
    
    # Create a prompt for the LLM to convert technical layout to natural language
    prompt = f"""[INST] You are an indoor navigation assistant. Convert the following technical 3D layout description into clear, human-readable navigation instructions that would help someone understand their spatial environment.

The technical layout contains walls, doors, windows, and objects with their coordinates. Convert this into natural language that describes:
- Room shape and size
- Location of walls relative to the person
- Where doors and windows are located
- Any furniture or objects present
- Navigation guidance

Technical Layout:
{truncated_text}

Provide a clear, concise description in simple English suitable for navigation assistance: [/INST]"""

    try:
        logger.info("Generating simplified layout description...")
        response = simplify_model(
            prompt,
            max_new_tokens=512,
            temperature=0.3,
            top_p=0.9,
        ...
        # Extract the response text (remove the prompt)
        simplified_text = response.split("[/INST]")[-1].strip()
        logger.info("Layout description simplified successfully.")
        return simplified_text
        
    except Exception as e:
        logger.error(f"Failed to simplify layout description: {e}")
        return f"Navigation Description: The space contains multiple walls, doors, and windows. {technical_layout_text[:500]}..."
...
# --- Rerun Visualization Function ---
def visualize_layout_with_rerun(point_cloud_path, layout_content, radius=0.01, max_points=1000000):
    """
    Integrated visualization function for Rerun 3D visualization.
    Returns a file path that Gradio's Rerun component can use.
    """
    try:
        logger.info(f"Starting visualization for {point_cloud_path}")
        
        # Initialize Rerun
        import rerun as rr
        import rerun.blueprint as rrb
        
        rr.init("spatiallm_visualization", spawn=False)
        ...
        # Log points to Rerun
        logger.info("Logging points to Rerun")
        rr.log(
            "world/points",
            rr.Points3D(
                positions=points,
        ...
        # Log bounding boxes for detected objects
        logger.info(f"Logging {len(floor_plan)} detected objects")
        ...
        # Create a success indicator file for Gradio
        viz_dir = os.path.join(output_dir, "visualizations")
        os.makedirs(viz_dir, exist_ok=True)
        
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        success_file = os.path.join(viz_dir, f"visualization_success_{timestamp}.txt")
        
        with open(success_file, 'w') as f:
            f.write(f"Visualization completed: {point_cloud_path}\n")
            f.write(f"Points: {len(points)}\n")
            f.write(f"Objects detected: {len(floor_plan)}\n")
        ...
        error_file = os.path.join(viz_dir, f"visualization_error_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt")
...
"""def generate_audio_from_text(text_input):
    ...
    try:
        ...
        # Clean up engine reference to prevent callback errors
        del engine
        ...
        # Create a fallback empty audio file
        with open(speech_file_path.replace('.mp3', '.txt'), 'w') as f:
            f.write(text_input)
        gr.Warning(f"Audio generation failed, saved text instead: {e}")
        return None
    """
...
def generate_audio_from_text(text_input):
    ...
    try:
        # Use a different approach to avoid engine conflicts
        import pyttsx3
        engine = pyttsx3.init()
        ...
        logger.error(f"Audio generation failed: {e}")
        # Alternative: use gTTS as fallback
        try:
...
def get_latest_frame():
    """Get the latest frame image for display in Gradio"""
    frame_files = [f for f in os.listdir(frame_dir) if f.endswith('.jpg')]
    if not frame_files:
        return None
    
    latest_frame_file = max(frame_files, key=lambda x: os.path.getctime(os.path.join(frame_dir, x)))
    frame_path = os.path.join(frame_dir, latest_frame_file)
    frame = cv2.imread(frame_path)
    if frame is not None:
        frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)  # Convert for Gradio display
    return frame
...
def gradio_interface():
    with gr.Blocks(title="SpatialLM Gradio | 3D Scene Understanding with Large Language Model ") as demo:
        ...
        radius_state = gr.State(value=0.01)
        max_points_state = gr.State(value=1000000)
        ...
        with gr.Row():
            ...
            with gr.Column(scale=1):
                ...
                #     },
                # )
                # In your gradio_interface() function, make sure the Rerun viewer is properly configured:
                rerun_viewer = Rerun(
                    streaming=True,
                    height=700,
                    panel_states={
                        "time": "collapsed",
                        "blueprint": "expanded",  # Show blueprint panel
                        "selection": "collapsed",
                    },
                )
                layout_text_output = gr.Textbox(label="Human-Readable Navigation Description", lines=8)
                audio_output = gr.Audio(label="Audio Output")
        
        def process_latest_pcd(last_processed_pcd_file, model_path, custom_prompt, top_k, top_p, temperature, num_beams, max_new_tokens, visualization_radius, max_visualization_points):
            pcd_dir = "outputs/pcd"
            if not os.path.exists(pcd_dir):
                return last_processed_pcd_file, None, None, None, None, None, None, None, None
            ...
            if latest_pcd_file != last_processed_pcd_file:
                ...
                try:
                    ...
                    # Simplify the technical layout description
                    ...
                    # Get corresponding frame for display
                    frame = get_latest_frame()
                    
                    # Prepare visualization - use the integrated function
                    visualization_data = visualize_layout_with_rerun(
                        latest_pcd_file, technical_layout_str, visualization_radius, max_visualization_points
                    )
                    
                    gr.Info("Pipeline completed successfully.")
                    return (latest_pcd_file, frame, latest_pcd_file, visualization_radius, 
                        max_visualization_points, technical_layout_str, simplified_layout_str, audio_path, visualization_data)
        ...
        process_outputs = process_btn.click(
            ...
            outputs=[
                ...
                layout_text_output, 
                audio_output,
                rerun_viewer  # Direct output to Rerun viewer
            ]
        )
        
        # Safe visualization function with error handling
        # Safe visualization wrapper
        def safe_visualize_layout(pcd_path, layout_text, radius, max_points):
            try:
                if pcd_path and layout_text:
                    logger.info(f"Visualizing PCD: {pcd_path}")
                    return visualize_layout_with_rerun(pcd_path, layout_text, radius, max_points)
                else:
                    logger.warning("Missing PCD path or layout text for visualization")
                    # Return a placeholder file
                    viz_dir = os.path.join(output_dir, "visualizations")
                    os.makedirs(viz_dir, exist_ok=True)
                    placeholder = os.path.join(viz_dir, "placeholder.txt")
                    with open(placeholder, 'w') as f:
                        f.write("Waiting for data...")
                    return placeholder
            except Exception as e:
                ...
                viz_dir = os.path.join(output_dir, "visualizations")
                os.makedirs(viz_dir, exist_ok=True)
                error_file = os.path.join(viz_dir, f"error_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt")
                with open(error_file, 'w') as f:
                    f.write(f"Visualization error: {str(e)}")
                return error_file

        # Chain the visualization after processing
        process_outputs.then(
            fn=safe_visualize_layout,
